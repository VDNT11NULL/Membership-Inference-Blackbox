...
flags.DEFINE_integer("num_samples", 500, "number of generated samples")
FLAGS = flags.FLAGS
...
def main(_):
    ...
    with tf.Session(config=run_config) as sess:
        ...
        if FLAGS.app == 'train':
            ...
            noise_sample = []
            img_sample = []
            output_folder = os.path.join(FLAGS.checkpoint_dir, "generated_images")
            ...
            np.savez_compressed(os.path.join(FLAGS.checkpoint_dir, 'generated.npz'), noise=noise_sample,
                                img_r01=img_r01)
            for idx, img in enumerate(img_r01):
            	img = (img * 255).astype(np.uint8)  
            	img_pil = Image.fromarray(img)
            	img_pil.save(os.path.join(output_folder, f"sample_{idx}.png"))
            print(f"Saved {FLAGS.num_samples} images to {output_folder}")
        else:
